...
class VibrationMode:
    ...
    def descr(self) -> str:
        return (
            f"{self.symmetry + ' ' if self.symmetry is not None else ''}{self.motion}"
        )
...
BONDS = {
    ...
    "CH2": Bond("CH₂", "CH₂"),
    "CH3": Bond("CH₃", "methyl"),
    "alpha-CH2": Bond("α–CH₂", "α–CH₂"),
    "alpha-CH3": Bond("α–CH₃", "α–CH₃"),
    "backbone-CH2": Bond("CH₂", "backbone CH₂"),
    "backbone-CH3": Bond("CH₃", "backbone CH₃"),
    "OCH3": Bond("OCH₃", "ester –O–CH₃"),
    "C=N": Bond("C=N", "azine"),
    "N-H": Bond("N–H", "N–H"),
    "C-N": Bond("C–N", "C–N"),
...
def find_vibration_mode_from_string(vib_str: str) -> VibrationMode:
    for vib in VIBRATION_MODES:
        if vib_str.replace("ing", "") in (
            vib.short().replace("ing", ""),
            vib.descr().replace("ing", ""),
            vib.symb(),
        ):
            return vib
    else:
...
def make_bond_vibration(bond_str: str, vib_str: str) -> BondVibration:
    return BondVibration(
        find_bond_from_string(bond_str), find_vibration_mode_from_string(vib_str)
    )
...
MATERIAL_LIBRARY = [
    ...
    Material(
        ...
        bands=[
            ...
            IRBand(make_bond_vibration("alpha-CH3", "asym bend"), 1485, 2.0),
            IRBand(make_bond_vibration("backbone CH2", "bend"), 1451, 2.0),
            IRBand(make_bond_vibration("OCH3", "asym bend"), 1433, 2.0),
            IRBand(make_bond_vibration("C-O-C", "asym stretch"), 1271, 5.0),
            IRBand(make_bond_vibration("C-O-C", "stretch"), 1240, 5.0),
            IRBand(make_bond_vibration("C-O-C", "sym stretch"), 1194, 5.0),
            IRBand(make_bond_vibration("C-O-C", "stretch"), 1151, 5.0),
        ],
    ),
...
MATERIAL_LIBRARY += [
    ...
    Material(
        ...
        "BSE",
        aliases=["protein standard", "albumin"],
        kappa=0.20,
        c=1800,
        rho=1350,
        n=1.45,
...
def get_material(identifier: str) -> Material:
    for mat in MATERIAL_LIBRARY:
        if (
            identifier in (mat.name, mat.shortname, *mat.aliases)
            or identifier in mat.aliases
        ):
            return mat
    return None
...
class AmbiguousIRBand:
    ...
    def descr(self) -> str:
        return ", ".join(
            [
                f"{mode.descr()} ({(' / '.join(mats))})"
                for mode, mats in self.modes_on_materials.items()
            ]
        )
    ...
    def __repr__(self) -> str:
        modes_on_mats_str = ", ".join(
            [
                f"{mode.__repr__()} : {mats}"
                for mode, mats in self.modes_on_materials.items()
            ]
        )
