...
class ComplexNormalize:
    ...
    def __init__(
        ...
        self.amin = float(amin) if amin is not None else None
        self.amax = float(amax) if amax is not None else None
        self.aoffset = float(aoffset) if aoffset is not None else None
        self.clip = clip
    ...
    def autoscale_angle(self, Z, coverage_limit=np.pi) -> None:
        ...
        est_offset_angles_min, est_offset_angles_max = (
            np.min(np.angle(est_offset_angles)),
            np.max(np.angle(est_offset_angles)),
        )
...
class ComplexColorTransformHSV(ComplexColorTransform):
    ...
    def __init__(
        ...
        mag_to_s: callable = None,
        mag_to_v: callable = None,
        mag_to_a: callable = None,
    ):
        self.name = name
        self.angle_to_h = (
            angle_to_h if angle_to_h is not None else lambda a: np.zeros_like(a)
        )
...
class ComplexColorTransformLCh(ComplexColorTransform):
    ...
    def __init__(
        ...
        self.angle_to_h = (
            angle_to_h if angle_to_h is not None else (lambda a: 360.0 * a)
        )

        # perceptual lightness (safe range!)
        self.mag_to_L = (
            mag_to_L
            if mag_to_L is not None
            else (
        ...
        self.mag_to_C = (
            mag_to_C if mag_to_C is not None else (lambda m: 40 * np.ones_like(m))
        )

        # alpha
        self.mag_to_a = (
            mag_to_a if mag_to_a is not None else (lambda m: np.ones_like(m))
        )
